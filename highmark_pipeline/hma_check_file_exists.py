...
def hma_check_file_exists(config=None, **objects):

    NAMESPACE = get_env("NAMESPACE", "claims-audit", False)
    DOMAIN_NAME = get_env("DOMAIN_NAME", "enterprise.xpms.ai", False)

    file_path = "minio://{0}/claimsaudit-ingestfiles/batches-input".format(NAMESPACE)
    xr = XpmsResource()
    ...
    if minio_resource.exists():
        ...
        #     }
            ...
            mr.copy(lr)
            backup_path = "minio://{0}/claimsaudit-ingestfiles/batches-inprogress".format(NAMESPACE)
