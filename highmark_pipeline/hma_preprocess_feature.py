...
def hma_preprocess_feature(config=None, **objects):
    ...
    for code in CLM_SAC_CD_LIST:
        mask = df[["CLM_SAC_CD_1", "CLM_SAC_CD_2", "CLM_SAC_CD_3", "CLM_SAC_CD_4", "CLM_SAC_CD_5"]].isin([code]).any(
            axis=1)
    ...
    file_path = "minio://claims-audit/ca_1efc74ee-4b8d-46f3-a703-5fd67bc22384/ml/label_encoder.pkl"
    ...
    final_df = pd.concat([ip_df, enc_df_transformed, ohe_dfs], axis=1)
    final_df.drop('CLAIM_NUMBER_Mask', axis=1, inplace=True)

    print("final shape:-", final_df.shape)

    file_name = objects["document"][0]["metadata"]["properties"]["filename"]
    csv_minio_urn = "minio://claims-audit/ca_1efc74ee-4b8d-46f3-a703-5fd67bc22384/ml_input/mapped_" + file_name
