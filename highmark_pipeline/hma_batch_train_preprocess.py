...
def hma_batch_train_preprocess(config=None, **objects):
    ...
    if "cas_stus_dscr" in df:
        final_df["cas_stus_dscr"] = df["cas_stus_dscr"]
        final_df["cas_stus_dscr"] = final_df["cas_stus_dscr"].map({"CAF": 1, "CFE": 0})
    else:
        final_df["ec_dscr"] = df["ec_dscr"]
        final_df["ec_dscr"] = final_df["ec_dscr"].map(
            {'Coding Error': 0, 'Frequency - Claim Error': 1, 'Frequency - Money Claim Error': 2, 'Internal Error': 3})

    print("final shape:-", final_df.shape)

    file_name = config['context']['input_file_name']
    csv_minio_urn = "minio://{}/ml_input/mapped_train_batch_".format(NAMESPACE) + file_name
    local_csv_path = "/tmp/mapped_train_batch_" + file_name
